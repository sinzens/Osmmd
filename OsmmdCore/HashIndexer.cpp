...
void Osmmd::HashIndexer::Insert(const ColumnValue& key, std::shared_ptr<RowValue> value)
{
    m_hashMap.insert({ ColumnValueToBytesForHash(key), value });
}

void Osmmd::HashIndexer::Delete(const ColumnValue& key)
{
    m_hashMap.erase(ColumnValueToBytesForHash(key));
}

void Osmmd::HashIndexer::Update(const ColumnValue& key, std::shared_ptr<RowValue> value)
{
...
std::vector<Osmmd::RowValue> Osmmd::HashIndexer::Select(const ColumnValue& key) const
{
    auto target = m_hashMap.find(ColumnValueToBytesForHash(key));
    ...
    return std::vector<Osmmd::RowValue>();
}

std::vector<Osmmd::RowValue> Osmmd::HashIndexer::Select(std::function<bool(std::shared_ptr<RowValue>)> filter) const
{
